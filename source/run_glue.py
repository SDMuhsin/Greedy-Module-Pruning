...
class DataTrainingArguments:
    ...
    job_id: str = field(default="JOBIDNOTSPECIFIED")
...
def main():
    ...
    if(model_args.prune_method == 'none'):
        ...
        print(f"Pruned {str(layers_to_prune)}")
        model.prune_layers(layers_to_prune)
    elif(model_args.prune_method == "activation-similarity"):
        #current_path = pathlib.Path(__file__).parent.absolute()
        layer_file_path = os.path.join("./", "layer_files/", f"{model.name_or_path}_{data_args.task_name}_activation.txt")
        with open(layer_file_path, 'r') as f:
            layers_to_prune = f.readlines()
        layers_to_prune = layers_to_prune[:model_args.prune_n_layers]
        layers_to_prune = [int(l.replace("\n", "")) for l in layers_to_prune]
    ...
    save_data[str(training_args.seed)] = eval_result
